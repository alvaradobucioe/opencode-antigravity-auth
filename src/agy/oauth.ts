...
async function fetchProjectID(accessToken: string): Promise<string> {
  const errors: string[] = [];
  const agyHeaders = getAntigravityHeaders();
  const loadHeaders: Record<string, string> = {
    Authorization: `Bearer ${accessToken}`,
    "Content-Type": "application/json",
    "User-Agent": agyHeaders["User-Agent"],
    "X-Goog-Api-Client": agyHeaders["X-Goog-Api-Client"] ?? "google-cloud-sdk vscode_cloudshelleditor/0.1",
    "Client-Metadata": agyHeaders["Client-Metadata"],
  };
